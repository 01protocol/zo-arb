...
import { ConfirmOptions, Connection, Signer, PublicKey, Transaction } from '@solana/web3.js';
import {
    BN,
    calculateMarkPrice,
    ClearingHouse,
...
export class DriftArbClient {
    ...
    async getCanOpenShort() {
        if (this.user.getPositionSide(this.user.getUserPosition(this.marketIndex)) == PositionDirection.LONG) {
            return true
        }
        return (convertToNumber(this.user.getPositionValue(this.marketIndex), QUOTE_PRECISION) < MAX_POSITION_SIZE)
    }

    async getCanOpenLong() {
        if (this.user.getPositionSide(this.user.getUserPosition(this.marketIndex)) == PositionDirection.SHORT) {
            return true
        }
        return (convertToNumber(this.user.getPositionValue(this.marketIndex), QUOTE_PRECISION) < MAX_POSITION_SIZE)
    }
