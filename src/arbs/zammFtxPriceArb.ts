...
export const runZammFtxPriceArb = async (log: any) => {
  ...
  while (true) {
    ...
    // TODO: calculate xyk
    // TODO: calculate if arbable
    // TODO: calculate how much to arb by
    // const [bids, asks] = await zoArbClient.getZammBidAskOrders();
    // const zoBid = await zoArbClient.getAverageTopBid(bids, POSITION_SIZE_USD);
    // const zoAsk = await zoArbClient.getAverageTopAsk(asks, POSITION_SIZE_USD);
    ...
    //
    // log.info(
    //   `Buy FTX / Sell Zamm ${process.env.MARKET}: ${ftxLongDiff.toFixed(4)}`
    // );
    // log.info(
    //   `Buy Zamm / Sell FTX ${process.env.MARKET}: ${ftxShortDiff.toFixed(4)}`
    // );
    //
